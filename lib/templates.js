...
function renderCatalogCard(entry, baseUrl) {
  ...
  const covered = entry.stages.filter((stage) => stage.state === 'active').length;
  ...
  const searchText = [entry.title, entry.description, entry.type, entry.level, entry.owner, ...entry.audience]
    .filter(Boolean)
    .join(' ')
    .toLowerCase();

  return `<article class="browse-card" data-browse-card${filterData}${audienceData} data-search="${escapeHtml(searchText)}">
    ...
    <div class="browse-card-stages">
      <p class="browse-card-stages-label">Delivery stages<span>${covered} of ${STAGES.length}</span></p>
