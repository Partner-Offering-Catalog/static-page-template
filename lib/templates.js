...
function renderCatalogCard(entry, baseUrl) {
  ...
  const coveredStages = entry.stages.filter((stage) => stage.state === 'active');
  ...
  const stageData = coveredStages.length
    ? ` data-stage="${escapeHtml(coveredStages.map((stage) => stage.title).join('|'))}"`
    : '';
  const searchText = [
    entry.title,
    entry.description,
    ...
    entry.owner,
    ...entry.audience,
    ...coveredStages.map((stage) => stage.title),
  ]
    .filter(Boolean)
    .join(' ')
    .toLowerCase();
  ...
  return `<article class="browse-card" data-browse-card${filterData}${audienceData}${stageData} data-search="${escapeHtml(searchText)}">
    ...
    <div class="browse-card-stages">
      <p class="browse-card-stages-label">Delivery stages<span>${coveredStages.length} of ${STAGES.length}</span></p>
...
function renderCatalog(entries, baseUrl) {
  ...
  const stageOptions = STAGES.map((stage) => ({
    ...
    count: entries.filter((entry) =>
      entry.stages.some((declared) => declared.id === stage.id && declared.state === 'active'),
    ).length,
  ...
  const legend = STAGES.map(
    (stage) =>
      `<li><span class="stage-badge stage-badge-active" aria-hidden="true">${stage.number}</span>${escapeHtml(stage.title)}</li>`,
  ).join('');
...
function renderFrameworkTable() {
  const rows = STAGES.map(
    (stage) => `<tr>
      ...
      <td data-label="Default anchor"><code>${escapeHtml(stage.defaultAnchor)}</code></td>
