  ...
  const items = resources
    .map((resource) => {
      ...
      return `<li class="resource">
        ...
        <p class="resource-tags">${type}${audience}</p>
        ${link}
      </li>`;
    })
    .join('');
...
function renderPage({ config, root, node, parent, ancestors, contentHtml, isOffering }) {
  ...
  if (isHome) {
    ...
    <article class="content-card${isOffering ? ' offering-card' : ''}">
      ...
      <div class="prose${isOffering ? ' prose-wide' : ''}">${contentHtml}</div>
    </article>
