...
/**
 * Folders reserved for files that are copied verbatim (decks, PDFs, images)
 * rather than turned into navigation entries.
 */
const ASSET_DIR = 'assets';

/**
 * Path of a content file relative to the content root, always posix-style so
 * it can be used to build URLs and resolve relative Markdown links.
 * @param {string} contentRoot
 * @param {string} filePath
 * @returns {string}
 */
function toContentPath(contentRoot, filePath) {
...
if (!fs.existsSync(readmePath)) {
    const relativeDir = path.relative(contentRoot, dirPath) || '.';
    throw new Error(`Missing required README.md in content folder: ${relativeDir}`);
  }
  ...
  const node = {
    ...
    body,
    slug,
    contentPath: toContentPath(contentRoot, readmePath),
    urlPath,
  ...
  for (const entry of entries) {
    ...
    if (entry.isFile() && entry.name.toLowerCase().endsWith('.md') && entry.name !== 'README.md') {
      ...
      node.children.push({
        ...
        body: pageBody,
        slug: pageSlug,
        contentPath: toContentPath(contentRoot, path.join(dirPath, entry.name)),
        urlPath: pageUrlPath,
...
module.exports = { buildContentTree, flattenTree, ASSET_DIR };
