 ...
 *
 * The framework names the stages and nothing else. Timing anchors belong to
 * the offering, which sets them with each stage's `Timing` field: a shared
 * default would be an anchor nobody chose, quietly standing in for the one
 * this engagement actually runs to.
 */
const STAGES = [
  {
    id: 'engage',
    ...
    core: true,
    summary:
      'Position the offering with the stakeholders who can fund and staff it, qualify it out early if it does not fit, and turn interest into a written commitment: named participants, locked dates, agreed outcomes.',
  },
  {
    id: 'scope',
    number: 2,
  ...
  },
  {
    ...
    core: true,
    summary:
      'Dry-run on the environment participants will actually use and take an explicit go/no-go decision, then deliver the engagement and capture evidence of progress as it happens.',
  },
  {
    id: 'wrap',
    number: 5,
    ...
    core: true,
    summary:
      'Capture outcomes, demos, and feedback while participants are still in the room, deprovision the environment, then track the commitments made until the value shows up.',
  },
];
 ...
 * an instruction.
 */
const MERGED_STAGE_TITLES = new Map([
...
/**
 * Explains an unrecognised stage heading. A title from the earlier framework
 * names the stage that absorbed it, so the fix is a rename rather than a hunt
 * through the stage list.
 *
 * @param {string} title The heading as the author wrote it.
 * @returns {string}
 */
function describeUnknownStage(title) {
  const successorId = MERGED_STAGE_TITLES.get(normalizeKey(title));
  const successor = successorId ? STAGE_BY_ID.get(successorId) : null;
  if (successor) {
    return `"${title}" was merged into stage ${successor.number} "${successor.title}". Use that heading instead.`;
  }
  return `"${title}" is not a framework stage. Use one of: ${STAGES.map((stage) => stage.title).join(', ')}`;
}
...
function parseOffering(body, options = {}) {
  ...
  for (const segment of frameworkSegments.slice(1)) {
    if (segment.level === 3) {
      ...
      if (!definition) {
        errors.push(`${source}: ${describeUnknownStage(segment.title)}`);
        currentStage = null;
        currentSection = null;
        continue;
      }
...
/**
 * Reads the catalog metadata an offering contributes to the overview page.
 *
 * The overview answers one question per stage: does this offering represent
 * it? A stage counts as represented when the offering declares it *and* wrote
 * something under it. A stage that is absent, declared but still empty, or
 * declared `Not applicable` is not represented — the offering page is where
 * that difference is explained, so the overview does not carry it.
 *
...
function toCatalogEntry(node, offering) {
  ...
  return {
    ...
    stages: STAGES.map((stage) => ({
      ...
      number: stage.number,
      title: stage.title,
      represented: declared.get(stage.id)?.state === 'active',
    })),
  };
}
